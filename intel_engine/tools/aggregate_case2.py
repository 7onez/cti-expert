...
def main(argv=None):
    """Aggregate one case. Import-safe: nothing here runs until main() is called."""
    # Resolved INSIDE main so that importing this module never reads argv, never picks
    # a case, and never writes. The old module-scope form defaulted to a REAL case id,
    # so a bare import silently rewrote that case's deliverables.
    args = sys.argv[1:] if argv is None else list(argv)
    CASE = args[0] if args else "CASE-0001"
    if os.sep in CASE or (os.altsep and os.altsep in CASE) or CASE in (os.pardir, os.curdir) or CASE.startswith(os.pardir):
        sys.exit(f"aggregate_case2: refusing case id with a path separator: {CASE!r}")
