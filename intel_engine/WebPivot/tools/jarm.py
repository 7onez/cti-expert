...
if __name__ == "__main__":
    import sys
    # cti-expert: this is a DISPATCH op (`intel.py jarm <host> [port]`), so a flag-shaped or
    # missing argument has to fail loudly. Upstream calls jarm as a library and took argv[1]
    # verbatim — which turned `--help` into a hostname, failed the TLS probe, and printed a
    # perfectly plausible all-zero fingerprint. In an intel pipeline a fabricated fingerprint
    # that looks like a real reading is worse than a crash: it can be recorded as a finding.
    if len(sys.argv) < 2 or sys.argv[1].startswith("-"):
        print("usage: jarm.py <host|ip> [port]     (default port 443)\n"
              "       JARM TLS-stack fingerprint — same JARM = same server stack/config.\n"
              "       Not an operator pivot on its own: shared hosting shares a JARM.",
              file=sys.stderr)
        sys.exit(2)
    h = sys.argv[1]
